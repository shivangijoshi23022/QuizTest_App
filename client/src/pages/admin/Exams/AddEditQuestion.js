...
function AddEditQuestion({
    ...
    if (uploadedFile) {
      ...
      reader.onload = async (event) => {
        try {
          ...
          await uploadQuestions(json);
        } catch (error) {
          message.error("Invalid JSON file format: " + error.message);
        }
      };
  ...
  // Function to upload questions from JSON
    ...
    try {
      for (const question of questions) {
        ...
        if (!response.success) {
          ...
          return; // Stop if any question fails
        }
      }

      message.success("All questions uploaded successfully!");
      refreshData();
      setShowAddEditQuestionModal(false);
    } catch (error) {
      message.error("Error uploading questions: " + error.message);
    } finally {
      dispatch(HideLoading());
    }
  };
